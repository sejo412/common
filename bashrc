...
parse_exit_code() {
  local EXIT="$?"
  if [ $EXIT == 0 ]; then echo -e "\033[01;32m$(date "+%H:%M:%S")"; else echo -e "\033[01;31m$(date "+%H:%M:%S")"; fi
}
...
export PATH="/usr/local/opt/sphinx-doc/bin:/usr/local/opt/coreutils/libexec/gnubin:/usr/local/opt/gnu-sed/libexec/gnubin:/usr/local/opt/findutils/libexec/gnubin:/usr/local/opt/grep/libexec/gnubin:/usr/local/bin:/usr/local/go/bin:$PATH:$HOME/.rvm/bin:$HOME/go/bin"
